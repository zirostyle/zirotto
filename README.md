...
### 자동화된 로또 구매 워크플로우
1. **잔액 확인** - 구매가능 금액 조회
2. **조건부 충전** - 잔액 10,000원 미만 시 자동 충전 (20,000원)
3. **연금복권 720+ 구매** - 5장 자동 구매 (5,000원)
4. **로또 6/45 구매** - 5장 자동 구매 (5,000원)
...
**자동 실행 (스케줄):**
- 이미 **매주 월요일 오전 9시 KST**로 설정되어 있습니다
- 스케줄을 변경하려면 `.github/workflows/purchase.yml` 파일의 `cron` 설정을 수정하세요

```yaml
# 현재 설정: 매주 월요일 오전 9시 KST
schedule:
...
# 매주 토요일 오후 8시 KST  
- cron: '11 0 * * 6'  # UTC 00:11 토요일 = KST 09:11 토요일
